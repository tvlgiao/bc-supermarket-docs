...
| Feature | Default state (production) | Enable method |
|---|---|---|
...
> Inside the BigCommerce **theme editor preview panel**, any widget you haven't configured in Script Manager renders with demo content so merchants can evaluate the feature before enabling it. See [Theme Editor Preview](#theme-editor-preview) below. Production shoppers always see the defaults above until you opt-in.
...
- **Only inside the editor.** Detection requires both an iframe context and the `?stencilEditor=` URL param that BigCommerce adds to every editor preview URL. Production shoppers never see demo content.
