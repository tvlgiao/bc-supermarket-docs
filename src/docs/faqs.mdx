...
## Show the discounted shipping price with strikethrough on the cart shipping estimator

When a shipping discount or promotion applies, the cart's **Estimate Shipping** box shows the full shipping price instead of the discounted one. This snippet displays the original price with a strikethrough next to the discounted price (e.g. ~~$19.99~~ $9.99). Shipping methods without a discount are left unchanged.

This is available natively in the latest theme version (it shows `cost_after_discount` returned by BigCommerce). Use this snippet only if you cannot update the theme. It reads the shipping-quote response the theme already requests, so it does not make any extra network calls; it does nothing when there is no shipping discount.
...
(function () {
    ...
    var QUOTE_PATH = '/remote/v1/shipping-quote';
    var APPLIED_ATTR = 'data-shipping-discount-applied';
    var origFetch = window.fetch;

    if (typeof origFetch !== 'function') { return; }

    var style = document.createElement('style');
    style.innerHTML = '.shipping-quotes .estimator-form-input--price .price--discounted { text-decoration: line-through; opacity: .7; margin-right: .35em; }';
    document.head.appendChild(style);
    ...
    window.fetch = function (input, init) {
        ...
        if (url.indexOf(QUOTE_PATH) === -1) { return result; }

        return result.then(function (response) {
            try {
                response.clone().json()
                    .then(function (json) {
                        var quotes = json && json.data && json.data.quotes;
                        if (quotes) { decorateWhenReady(collectDiscounted(quotes)); }
                    })
                    .catch(function () {});
            } catch (e) {}
            return response;
    ...
    function collectDiscounted(quotes) {
        var map = {};
        ['shipping_methods', 'ups_shipping_methods'].forEach(function (group) {
            (quotes[group] || []).forEach(function (m) {
                if (m && m.cost_after_discount && m.cost && m.cost.formatted) {
                    map['shipping-quote-' + m.type + '-' + m.id] = m;
                }
            });
        });
        return map;
    ...
    function decorateWhenReady(map) {
        if (!Object.keys(map).length) { return; }
        var attempts = 0;
        (function attempt() {
            var remaining = decorate(map);
            attempts += 1;
            if (remaining && attempts < 25) { setTimeout(attempt, 100); }
        })();
    }
    ...
    function decorate(map) {
        ...
        Object.keys(map).forEach(function (id) {
            ...
            var row = input && input.closest('.estimator-form-row');
            var label = row && row.querySelector('.estimator-form-input--price label');

            if (!label) { pending = true; return; }
            if (label.getAttribute(APPLIED_ATTR)) { return; }
            if (label.querySelector('.price--discounted')) { label.setAttribute(APPLIED_ATTR, '1'); return; }

            var m = map[id];
            label.innerHTML =
                '<span class="price--discounted"><b>' + esc(m.cost.formatted) + '</b></span>'
                + '<span><b>' + esc(m.cost_after_discount.formatted) + '</b></span>';
            label.setAttribute(APPLIED_ATTR, '1');
        });
        return pending;
    }
...
## Display non-image products last on the product lists
...
## Display custom social icons on the footer
...
## Tweaks Brands Carousel autoplay
...
You can change the number of subcategories shown by default by editing the number in line `var LIMIT = 10;`.


## Replace On Sale product badge by image
...
Upload your badge image in Storefront > Image Manager. Replace `/product_images/uploaded_images/low-everyday-price.png` by your badge image URL.


## DNS Prefetch and custom CSS for Page Speed Optimization
...
![Align product cards evenly](img/align-product-card-evenly.jpg)
...
## Display asterisks (*) beside the required fields
...
## Show "you save" price = regular price - sale price
...
## Make the search form sticky on mobile
...
## Display Bulk Pricing table on every product cards
...
## Hide Additional Information tab on PDP when it is empty
...
## Install Pinterest Tag code to the checkout page
...
Replace `INSTAGRAM_TAG_ID` by your own Instagram Tag ID.

Reference:

- https://developers.pinterest.com/docs/ad-tools/conversion-tag/
- https://help.pinterest.com/en/business/article/bigcommerce-and-pinterest-tag



## Fix page auto scrolled down when opened

Currently there is a problem with Facebook SDK which causes anonymous user cannot view your Facebook page for example: https://www.facebook.com/AlFlahertys/
screenshot: http://prntscr.com/vj2vko and the page automatically scrolled down to the Facebook Chat Box.

To fix this issue, please try to disable Facebook Chat Box in **Theme Editor** > **Social Media Icons** > **Facebook page**, Clear your facebook page name.
Screenshot: http://prntscr.com/vj3gk2


## Fix blog post alignment issue in version 4.8.0
...
![Display search icon on mobile](img/display-search-icon-on-mobile.jpg)
...
## Display GeoTrust, Norton, McAfee seals on the checkout page
...
## Fix Search in Category form overlaps the widget banner on mobile
...
## Improve Cookie Consent banner appearance
...
### Center and bigger popup on desktop:
...
## Display Coupon / Gift Certificate box and on the checkout page
...
## Display AfterPay "or 4 interest-free installments" message

Folow this guide: https://developers.afterpay.com/afterpay-online/docs/javascript-library


## Display Klarna "4 interest-free payments of" on-site message

Reference:
- https://developers.klarna.com/documentation/on-site-messaging/customization/
- https://x.klarnacdn.net/plugins/Klarna%20On-Site%20Messaging%20on%20BigCommerce%20-%20Support%20Guide.pdf
...
Replace `YOUR_MERCHANT_ID` by your Merchant ID code.

2. Edit a product page in Page Builder, add a HTML widget to a particular global region. Enter the content below to the HTML widget:
...
## Display all cart items in the dropdown cart preview

Edit file **templates/components/common/cart-preview.html**, change line: `{{#each (limit cart.items 4)}}` to `{{#each cart.items}}`.
...
## Redirect customers to the login page if not logged in
...
## Add Image Cloud Zoom to product images on PDP
...
## Add jQuery Zoom to product images on PDP
...
## Display products by category image in GIF format
...
## Hide Shipping Nofitication on PDP when product is out of stock or not track
...
## Display additional checkout buttons full width
...
## Display page content full width when the sidebar is empty
...
# Display level 3 sub-menu items of the standard-style menu in multiple columns
...
## Fix an issue with the button icon in version 5.1.0
...
## Add links to Frequently Bought Together products
...
## Display the quantity box on product cards

Kindly contact us for a quote on this customization.
...
![sticky add to cart](img/sticky-add-to-cart.jpg)
...
## Display the included VAT/Tax price below the excluded Tax price
...
## Display an alert message after customer select a specific shipping country
...
## Display a custom message below the order number on the Order Confirmation page
...
## Fix anchor links do not work on category pages if the Faceted Filters is enabled
...
## Customize Mega Menu Banners for each category menu item

![custom mega menu banners](img/custom-mega-menu-banners-800.jpg)
...
## Display recent blog posts on the homepage in 4 columns
...
## Display Phone Number mask on the checkout page
...
## Change color of the info alert message
...
Where `color: #0000ff` is the text color, `background-color: #f2f2f2` is the background color, `fill: #0000ff` is the icon color.


## Always display shipping address form on the checkout page
...
## Hide all additional checkout buttons on all pages
...
The issue of incorrect indents in UL, OL, and LI elements in product descriptions arises due to an erroneous HTML structure, such as a P element nested within an LI element. To resolve this:
...
![custom mega menu column](img/custom-mega-menu-column-break.png)
...
## Hide Quick-View, Add to Cart and Compare icons on product cards
...
## Fixing Quick Payment Button Sizing on Mobile

If you're encountering issues with the size of Quick Payment buttons on mobile devices during checkout, or within the cart preview popup on desktop, follow these steps to resolve the problem:
...
(function() {
var style = document.createElement('style');
style.innerHTML = '@media (max-width: 550px) { #googlepaypaypalcommerceCheckoutButton { grid-column: span 12 } }'
+ '.previewCart-additionalCheckoutButtons { display: flex; flex-direction: column }';
document.head.appendChild(style);
})();
</script>
```
...
## Configure Affirm Messages Properly

To properly configure Affirm payment settings and display Affirm messages on product details and cart pages, follow these steps:

### Step 1: Configure Affirm Payment Settings

Go to **Settings** > **Payments** > **Affirm settings** > **Advanced settings** and configure the following:

**Product pages price selector:**

Use one of these CSS selectors depending on your tax display settings:

```css
.productView-price .price--withoutTax.price--main
```

or

```css
.productView-price .price--withTax.price--main
```

### Step 2: Add Cart Styling Script

To ensure proper display of Affirm messages in the cart, add the following script:

Go to **Storefront** > **Script Manager**, click **Create a Script**, and configure:

- **Location on page** = `Footer`
- **Select pages where script will be added** = `All Pages`
- **Script type** = `Script`

Enter the script below in **Scripts contents**:

```html
<script>
(function() {
    var el = document.querySelector('[data-cart-totals]');
    if (el) {
        el.classList.add('cart-content-padding-right');
    }
})();
</script>
```

This script adds proper padding to the cart totals section to accommodate Affirm messaging display.


## Stock level not showing on product page for some products or variants

**Symptom:** Even though **Advanced Settings** > **Inventory** > **Stock level display** is set to `Show stock levels`, the stock number is missing on the product page for some products — or it disappears after selecting a variant (e.g. picking a color/size swatch), while other products show it correctly.

**Cause:** This is a known theme bug affecting the built-in stock-level display on the product page. It is fixed in the next theme update.

**Workaround:** Enable the **Social Proof** PDP Urgency widget with only the stock level turned on. The urgency widget renders the stock level reliably; setting `viewCount` and `lastOrder` to `false` keeps the random "people viewing" and "last ordered" messages hidden, so shoppers only see the stock number.
...
This shows the stock level on the product page but does not display the random view count or last-order messages.

> **Note:** If you already use a `window.supermarketThemeSettings` snippet for other Social Proof features, do not add a second one — merge the `urgency` key into your existing snippet instead. See [Social Proof — Starter snippet](/customization/social-proof#starter-snippet) and the [PDP Urgency Widget](/customization/social-proof#pdp-urgency-widget) reference for all available options.


## Fix loading skeleton stuck in the "Added to cart" popup

When you add a product from a product **card/grid** (home, category or brand pages), the "X item(s) added to your cart" popup may show three grey placeholder bars (a loading skeleton of the Cart Shipping Goal Bar) stuck at the top forever.

This happens because adding from a card goes through a different code path than adding from the product page, and — in older theme versions — that path did not notify the Cart Goal Bar that the popup had finished loading. The bar is therefore never hydrated (when the feature is enabled) nor hidden (the default), so its skeleton lingers.

This is already fixed natively in the latest theme version. Use this snippet only if you cannot update the theme yet. It makes no network calls, and it watches only the "Added to cart" popup element (idle until you add a product), so it adds no ongoing scanning cost to the rest of the page. **Remove it after you update the theme.**
...
(function () {
    ...
    // The "Added to cart" popup is a static container present at page load;
    // only its inner content is replaced per add. Observing just this element
    // (instead of document.body) keeps the observer idle until a product is
    // added — no page-wide mutation scanning.
    var modal = document.getElementById('previewModal');
    if (!modal || !window.MutationObserver) { return; }

    var SELECTOR = '[data-papathemes-cart-goal][aria-busy="true"]:not([data-sm-goal-fixed])';

    function notifyCartUpdated() {
        try {
            document.dispatchEvent(new CustomEvent('supermarket:cart-updated', {
                detail: { subtotal: null },
            }));
        } catch (e) {
            // Never let this break the cart preview.
        }
    }

    function scan() {
        var pending = modal.querySelectorAll(SELECTOR);
        if (!pending.length) { return; }
        // Mark first so the event handler's DOM changes can't re-trigger us.
        for (var i = 0; i < pending.length; i++) {
            pending[i].setAttribute('data-sm-goal-fixed', '1');
        }
        // Handler re-reads data-subtotal from the freshly injected element.
        notifyCartUpdated();
    }
    ...
    new MutationObserver(function () {
        if (scheduled) { return; }
        scheduled = true;
        // Coalesce the burst of mutations from injecting the popup HTML.
        setTimeout(function () {
            scheduled = false;
            scan();
        }, 50);
...
## Show full product titles on the homepage (disable truncation)

By default, the homepage's Featured / New / Best-selling tabs and the Products by category tabs clamp product titles to 2 lines so every card in a row keeps the same height, while the rest of the site (category, search and product carousels) already shows full titles. The snippet below restores full, untruncated titles on the homepage as well.
...
We use **Header** here (not Footer) so the styles apply before the page first paints, otherwise the titles would briefly show clamped to 2 lines and then jump to full length.
...
(function() {
    var css = document.createElement('style');
    css.innerHTML = `
        .emthemesModez-specialProductsTabs .card-title > a,
        .emthemesModez-productsByCategoryTabs .card-title > a {
...
The theme's `min-height` on the title is left untouched, so 1-line titles still reserve the 2-line height and stay aligned with taller cards in the same row.

If you prefer a hard cap (the "at least 3 or 4 lines" case) rather than fully unbounded titles, keep the theme's clamp but raise the line count, and bump the reserved height so short titles stay aligned. Replace the CSS inside the backticks above with:

```css
.emthemesModez-specialProductsTabs .card-title > a,
.emthemesModez-productsByCategoryTabs .card-title > a {
    -webkit-line-clamp: 3 !important; /* set 4 for four lines */
}
.emthemesModez-specialProductsTabs .card-title,
.emthemesModez-productsByCategoryTabs .card-title {
    min-height: 3.75em !important; /* ~3 lines; use ~5em for four lines */
}
```

Unbounded (first snippet) shows the complete title but rows can vary in height; the 3/4-line variant keeps a hard cap while showing more than the default 2 lines.

With full titles, each row's cards still stretch to the same outer height, but the inner rows (price, add-to-cart) may not line up across cards because the title line-count now varies. If any product carousel looks uneven, see **Make all product card items in carousel (Related Products, Customer Also Viewed) same height** above.


## Fix HTML tags showing in the store credit line in the mobile menu

In versions 7.3.0 to 7.5.0, a logged-in customer with store credit sees raw HTML in the mobile / tablet navigation menu instead of the formatted amount:

```
<SPAN CLASS='FIRSTWORD'>$212.59 USD</SPAN>STORE CREDIT
```

The desktop header is not affected. The store credit label contains a small `<span>` so the amount can be styled separately from the words "Store Credit", but the mobile menu escapes it, so the tag is printed as text. The same line also uses a fixed grey that does not match the other items in the menu — on a dark navigation it falls below the WCAG AA contrast minimum.

This is fixed in the theme from the release after 7.5.0. Until you update, the snippet below repairs it without editing any theme file.
...
We use **Footer** here so the menu markup already exists when the script runs. There is no visible flicker because the mobile menu is closed until the shopper taps the hamburger icon.
...
<script>
(function () {
    var RE = /^<span class=['"]firstword['"]>([\s\S]*?)<\/span>([\s\S]*)$/i;

    function fix() {
        var items = document.querySelectorAll('.navPages-action--storeCredit');
        if (!items.length) return;

        var ref = document.querySelector('.navPages-action:not(.navPages-action--storeCredit)');
        var navColor = ref ? getComputedStyle(ref).color : '';

        for (var i = 0; i < items.length; i++) {
            var el = items[i];
            if (el.hasAttribute('data-storecredit-fixed')) continue;

            var m = el.textContent.trim().match(RE);
            if (!m) continue;

            el.setAttribute('data-storecredit-fixed', '1');
            ...
            if (navColor) el.style.color = navColor;
        }
    }
...
The store credit line will read `$212.59 USD STORE CREDIT` on one line, in the same colour and weight as the ACCOUNT and SIGN OUT items below it. Hard-refresh on your phone, or use a private tab, to clear the cached page.

A few notes on how it behaves:

- The colour is read from the menu item next to it at runtime rather than hard-coded, so it follows whatever you set as the navigation colour in **Theme Settings**.
- The label is rebuilt from plain text using `createElement` and `textContent` rather than by assigning `innerHTML`, so the snippet cannot introduce a scripting issue.
- It only rewrites the line while the tags are still printed as text. Once you update to a theme version where this is fixed, the snippet does nothing — so it is safe to leave in place, and safe if it runs more than once.
- Nothing outside the mobile menu store credit line is touched.

If you prefer the amount on its own line above the words, matching the desktop header, add `span.style.display = 'block';` and `span.style.marginRight = '0';` in place of the existing `span.style.marginRight` line.
