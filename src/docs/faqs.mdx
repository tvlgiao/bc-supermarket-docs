...
(function($) {
    ...
    function main() {
        $('.productView').not('._bulkPricingMod').each(function(i, productView) {
            $productView = $(productView).addClass('_bulkPricingMod');
            $table = $productView.find('.productView-bulkPricing-table').first();
            $table.data('modified', true);
            var bulk = [{ min: 0, max: 0, percent: 0 }];
            var $price = $productView.find('[data-product-price-without-tax]').first().hide();
            var $priceClone = $price.clone().insertAfter($price).addClass('_clone').show();
            var $qty = $productView.find('[data-quantity-change] input');
            $table.find('tr').each(function(j, tr) {
                var $td;
                $td = $(tr).find('td').first();
                var item;
            ...
            var showBulkOptions = function() {
                $productView.find('.form-field--bulk').remove();
            ...
            $productView.find('[data-quantity-change] button').on('click', debounce(updatePrice, 300));
            updateBulkPrice();
            showBulkOptions();
        });
    }
...
Check this link to get the code: https://www.paypal.com/ph/webapps/mpp/logos-buttons

You can add the code to your website using BC HTML Widget.
